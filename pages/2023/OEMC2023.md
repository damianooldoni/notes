...
Open Earth Monitoring (lead by Tom Hengl) succeeded in gathering the most important European initiatives dealing with Earth Observation (EO) data. The EO community confirms to be very active and many initiatives are in development phase. Some challenges and attemptive solutions were discussed:
...
It was also clear how important is the [European Space Agency](https://www.esa.int/) (in collaboration with [NASA](https://www.nasa.gov/)) to build long term answers to such challenges. An example is the [Copernicus Data Space Ecosystem](https://dataspace.copernicus.eu/), which facilitates the access to both data and tools using open data and open science paradigms.
...
- cube based DGGS: no discontinuities, no gaps, no overlaps, hierarchical nesting, cell indexing, but moderate distortions and biggest disadvantage: no EPSG :scary:
